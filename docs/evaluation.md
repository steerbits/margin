...
The final `npm run test:release` battery passed in a disposable source/data copy:

- **337 Node tests passed**, including version ordering, quiet follow-ups to highlighted releases, unknown/mismatched running identities, malformed/offline/oversized feeds, conditional caching, retry/cooldown persistence, changed release identities, pinned prompts, release preparation/tagging, explicit test skips, failing/mutating checks, missing/untracked/escaped images, symlinked release/package paths, commit hooks, credential-isolated test environments, and registry completeness.
- **TypeScript, production, and independent recovery builds passed.** The existing Vite large-bundle advisory and Node experimental SQLite warnings remain.
- **228 Chromium/integration test executions passed** across six configurations: ordinary browser workflows **181**, gateway **13**, writable connection workflows **23**, connection gateway guards **2**, production source-send guards **8**, and two-installation discovery **1**. Some workflows intentionally run in multiple configurations; these are executions, not 228 unique scenarios.
- Update/Help checks exercise exactly-once autosend, rapid repeated Update clicks, pinned target reuse, original draft preservation, no-AI GitHub fallback, blocked source work with the original message, no delayed queuing, editing while readiness is pending, failed send/retry, visible creation errors, cached/offline information, unknown browser/server identity, and a 320px header.
- Two actual disposable frontend builds/hosts use fixture versions `1.0.0` and `1.1.0` with a cached test feed. The older build advertises the update; editing its source package version does not change its captured running identity or clear the badge. The matching newer build/startup reports `1.1.0` without a badge. This validates discovery/identity, not a historical Git upgrade or data migration.

The first complete attempt stopped after browser failures: eight customization setup timeouts and one workspace-Notes setup timeout matched previously documented startup/sidebar assumptions; two branding checks mistook the checkout path's literal `pi` for UI branding. Helpers now wait for bootstrap before deciding whether to toggle the sidebar, and branding checks exclude only the exact workspace path while preserving that displayed path. A later attempt exposed two stale onboarding tests expecting the conversation picker directly after workspace selection; they now exercise the existing workspace-home → New conversation → picker handoff. All corrected workflows passed in the final complete run. No test was skipped to obtain the pass.

Keeping disposable fixtures beneath `.margin-data/temporary/` also exposed Express's absolute `sendFile` dot-directory handling for SPA deep links. Native and gateway hosts now serve the fixed `index.html` relative to the fixed `dist` root, preserving dotfile protection. Deep-link regressions passed in both hosts.

Self-review inspected actual desktop Settings and 320px header screenshots. It also led to release-path/commit-hook checks, a credential allowlist for the battery environment, and making Settings-launched update creation errors visible. No independent reviewer or live-user preference study was available. Final machine-readable results are retained locally under `.margin-data/temporary/release-tests/run-6Zmitv/results.json`; validation logs are under `.margin-data/temporary/update-validation/`.

**Limits:** browser inference is scripted; gateway routing uses the existing cco shim, not real OS enforcement. No live provider inference, public GitHub discovery/push, actual agent-driven merge, data migration/rollback, Safari, or real mobile keyboard was exercised. The release battery reuses installed dependencies rather than performing a clean dependency installation. Prompts are guidance, not enforced read-only mode or evidence of model compliance. Source checkpoints do not protect all private data or ignored plugins. The committed feed deliberately has no published release; package version remains `0.1.0`. No live launcher was restarted or live production bundle rebuilt. Rebuild/restart normally and refresh to activate the feature; versions predating the checker need one manual upgrade.
...
- **291 Node tests passed**, including the real Pi SDK context-refresh fixture; **151 Chromium browser tests passed** and **12 gateway browser tests passed**. TypeScript, the production build, and independent recovery build passed. The existing bundle-size advisory remains (approximately 733 KB / 225 KB gzip).
- A real-SDK fixture captures actual assembled prompts from two already-created workspace sessions. It verifies global updates in both, workspace isolation, ancestor inheritance, alternate-file precedence and removal, no duplicated/stale text, and an unchanged prompt while a request is paused. Selected tools, skills, extensions, settings and conversation history remain intact. Invalid UTF-8 fails before input acceptance; repairing the file allows retry. A synthetic prompt-rebuild failure checks that activation is not incorrectly marked complete.
- File tests cover literal Unicode/BOM/CRLF, missing and empty files, 0600 global-file permissions, read-only files, links, oversized/invalid input, competing revisions, external edits, and temporary-file cleanup. Gateway tests cover authentication, hostile Origins, invalid target/body input, scope routing, and global saves without a functioning new-worker launcher.
- Nine instruction-focused browser workflows cover persistence, scope separation, clean/dirty Close and Escape, Back/Forward restoration, return destinations, the workspace picker, native-chooser guards, two-tab conflicts, failed saves, confirmed draft replacement, missing/read-failed files, Settings autosave independence, and native beforeunload dismissal. Existing Notes/navigation/model-picker tests were updated to exercise the newly separated workspace-home → model-choice → chat flow, not bypass it.
- Self-review inspected actual desktop and 390px screenshots. It found a stale preview after discarding a conflict and a mobile footer that overlapped the textarea. The preview now reloads the saved file; opening the editor reveals the complete edit group and its controls stay outside the independently scrolling textarea. Browser assertions cover the corrected geometry. Panel-only Back navigation also retains the mounted editor instead of hiding its guard behind a mobile Notes dialog. No independent reviewer or real-user preference study was available.
- One existing managed-shell guardian test failed once during parallel build/browser/unit execution (`dead` rather than `alive` before the deliberate SIGKILL). It passed in isolation and in the subsequent full 291-test run, without changes to managed-shell code or that test. This intermittent observation is retained rather than treated as a feature regression fix.

No live provider inference, actual cco OS-enforcement test, Safari/mobile-keyboard test, or live-app restart was performed. Gateway tests use the disposable routing shim, not an OS sandbox. Model compliance is not established by prompt-assembly tests. Instruction drafts do not survive a confirmed browser exit; external editors do not share Margin’s compare-and-commit transaction. Restart Margin normally and refresh the browser to activate the new routes and build. See [usage and limits](agents-instructions.md).
