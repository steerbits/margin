...
- **Save** adds a comment to the pending feedback batch; **Edit** reopens it for writing. **Enter** in a comment editor performs the same Save action (not Send); **Shift+Enter** inserts a newline. **⌘Enter / Ctrl+Enter** remain compatibility shortcuts for Save. In **Overall feedback**, Enter sends the attached batch when Send feedback is available; Shift+Enter inserts a newline. Unfinished typing is still automatically preserved as a recovery draft, but it is not attached until saved. Legacy nonempty draft comments remain attached after upgrading. Empty selections do not block sending.
