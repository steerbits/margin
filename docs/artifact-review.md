...
The **Artifacts** toolbar button reopens this conversation's existing reviews. It currently remains visible even when no artifacts are registered. There is no manual path/URL input in the review UI. Files still must be inside the workspace, and an app must already be running.
...
- Explicit deletion removes the review annotation from the UI. Feedback already sent remains in the conversation history. Currently, sent annotations also remain visible in the review after the artifact changes, but are not attached to later sends. Updates do not automatically resolve old notes; unfinished drafts are preserved. Sending currently leaves the review open.
