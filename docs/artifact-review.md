...
After rebuilding/restarting Margin, open a conversation and choose **Artifacts**, then **Open file or app**. Enter a workspace-relative/absolute `.md`, `.markdown`, `.html`, or `.htm` file, or an already running app such as `http://127.0.0.1:3000/settings`.
...
- **Page comment** works even if element selection is unavailable.
- Draft text saves automatically, including unfinished/empty comments. Closing the overlay preserves drafts and the selected artifact. Exact live app state is not restored after closing/reloading.
- **Retarget** attaches a draft to a newly selected passage/element. Missing/changed targets retain their saved quotation, route and comment. Nothing is automatically deleted or resolved.
- **Send comments** sends the current artifact's nonempty drafts as one user message in the associated chat. Comments on other artifacts remain drafts. While the agent is busy or awaiting a dialog, keep reviewing and send once ready; background queuing/steering is not implemented.
