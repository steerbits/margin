...
**Historical proposal.** Release tooling, cached discovery, quiet/highlighted notifications, autosent update reviews, and automatic startup builds for changed local versions are now described in [Releases and updates](releases.md). That implementation supersedes this note's unsent-draft interaction and manual-build-only assumptions; transactional installation/recovery improvements below remain proposals.
