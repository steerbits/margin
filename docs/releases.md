...
A feed with `latest: null` means no release has been published through that feed yet. Users on versions predating the checker need one manual upgrade before discovery works.
...
If the raw feed connection fails or times out, the same check tries the official GitHub contents API for `steerbits/margin/releases/stable.json` on `main`, requesting the raw file without authentication. Each endpoint has an eight-second request/body timeout, so a check using fallback can take up to sixteen seconds. ETags are scoped to their endpoint, including across restarts. HTTP errors and invalid manifests do not trigger fallback. Browser access can work while the backend fails because they use separate network connections; the browser does not supply the release feed to Margin.
...
**Activating source changes:** finish active tasks, stop the existing launcher, run `bash start.sh` (or `npm start`) with your usual port options, and refresh the browser. Production startup runs `npm run build` automatically if `dist` is missing, its version metadata is invalid, its version differs from `package.json`, or a previous startup build failed. It verifies the resulting build before starting the server. Matching builds skip this step; development mode and help/dry-run do not build. Same-version source edits still require `npm run build` before restarting. Startup uses local source and installed dependencies; it does not fetch releases or replace dependencies.

These triggers compare versions and required build files, not Git commits or arbitrary source edits. Same-version customizations, plugin additions and source restores still need an explicit build; older restored launchers and explicit legacy launch commands may also require it. See [startup checks and failure recovery](installation.md#existing-users). A public release becoming available does not itself change or rebuild the local installation.

Vite writes `dist/margin-build.json` and embeds the frontend version. Hosts capture identity at startup. If source and build versions differ (for example, on an explicit legacy launch), the known build version stays visible with a separate source/build warning. Browser/server version mismatches show an unknown-version/restart warning. Discovery and release review remain available even when the installed version is unknown; the summary still shows the latest published release or a check failure. Editing `package.json` or rebuilding under an existing host does not change its captured identity; a fresh startup and browser refresh are required. This records an upstream baseline, not proof that every customized file is identical to upstream. Development mode captures source version at startup and explicitly labels that limitation.
