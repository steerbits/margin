...
Startup shows loading indicators for workspaces/conversations instead of empty lists or a premature **Ready** label. A failed or timed-out bootstrap offers **Retry loading** without a page reload. On desktop, existing chats reveal the sidebar when no sidebar preference is saved; explicit collapse choices and mobile defaults are preserved.
...
Right-click a conversation in the sidebar and choose **Delete conversation** to delete its messages, comments, composer draft, uploaded attachments, and native Pi transcript permanently after confirmation. The same menu is available with Shift+F10 when a chat row has keyboard focus, or by long-pressing a row on touchscreens. On narrow or touch-capable screens, each row also has a **⋯** button for single-tap access. Moving or scrolling cancels a pending long-press; releasing a completed long-press does not open the chat. The first menu action is **Mark unread**, or **Mark as read** for an unread chat; active chats retain **Stop conversation**, and **Delete conversation** is always last. Stop running work and answer or cancel pending dialogs first. Deleting a chat keeps workspace files and shared plugin notes. Bookmarks to deleted chats show an unavailable destination.

Chat rows stay on one line without a chat icon or status text. A small spinner appears while the main agent is running, and a blue dot marks unread replies. The current chat’s header provides its detailed agent state. Summaries update approximately every 1.5 seconds across workspaces. Finished replies remain unread until the end of the latest reply is visible in the focused browser tab. Read markers persist in this browser across reloads; other browsers maintain their own read state. **Mark unread** also works for empty chats, and keeps the current chat open. Its reminder survives visibility checks and reloads until you explicitly select the chat again or choose **Mark as read**. Reopening resumes the normal latest-reply visibility check. These actions sync between tabs on the same origin; a new completion after **Mark as read** becomes unread again. They remain local to the browser origin, so switching ports can show previously read replies as unread; chat storage is unchanged. Restarted or exited workers report interrupted activity as stopped. Individual plugin background agents are not listed.
...
Choose a port with `bash start.sh --port 4318` or `npm start -- --port 4318`. Precedence is `--port`, then `PORT`, then the saved installation port, then 4317. If busy, the installer/launcher checks up to 10 higher ports and asks before switching; unattended launches fail with an explicit `--port` suggestion. Installation saves its accepted port; start-time choices do not change the saved default. One launcher is allowed per physical checkout, regardless of port or `MARGIN_DATA_DIR`; separate fresh checkouts may coexist. The ownership guard takes effect after restarting the launcher on this version. See [installation and port recovery](docs/installation.md).
